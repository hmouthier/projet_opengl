...
classification::classification(Mat *src)
{
        ...
        for( int x = 0; x < meanshift.cols; x++ )
        {
            if( mask.at<uchar>(y + 1, x + 1) == 0 ) // mask is offset from original image.
            {
                ...
                if (boundingRect->width > 35 && boundingRect->height > 35) {
                    ...
                    classifications.push_back(classification(ROI, alpha, boundingRect-> x - 1, boundingRect-> y - 1));
                }
    ...
    for(int i = 0; i <classifications.size(); i++) {
        ...
        copyPercept(classifications[i], reconstruction);
...
int classification::copyPercept(classification &unit, Mat &dest) {
